...
function CoachBubble({ message }: CoachBubbleProps) {
  const isAssistant = message.role === 'assistant';

  return (
    <View
      className={`flex-row mb-3 max-w-[85%] ${
    ...
    >
      ...
      <View
        className={`rounded-2xl px-4 py-3 max-w-full ${
          isAssistant
            ? 'bg-card rounded-bl-sm border-l-[3px]'
            : 'rounded-br-sm'
        }`}
        style={
          isAssistant
            ? { borderLeftColor: '#00C48C' }
            : { backgroundColor: mflColors.accent }
        }
      >
        <AppText
        ...
        <AppText
          className={`text-xs mt-1 ${
            isAssistant ? 'text-muted' : 'text-right'
          }`}
    ...
    return (
      ...
      >
        <ScreenState
          screen="ai-coach"
          state="empty"
          message="Select a league to chat with your AI Coach"
        />
      </View>
    );
  }
        ...
        {QUICK_PROMPTS.map((prompt) => (
          ...
          >
            <Chip size="sm" variant="soft" style={{ backgroundColor: mflColors.accentLight }}>
              <Chip.Label style={{ color: mflColors.accent }}>{prompt}</Chip.Label>
            </Chip>
          </Pressable>
        ))}
      </View>

      {/* Chat history (inverted = newest at bottom) */}
      <FlatList
        data={messages}
        ...
        ListEmptyComponent={
          ...
          >
            <AppText className="text-sm text-muted text-center">
              No messages yet. Ask your AI Coach anything about your fitness journey!
            </AppText>
          </View>
        }
      />
